...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Future<void> pickAndUpload(ImageSource source) async {
    try {
      ...
      await user.reload();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Cập nhật avatar thành công!')),);
      setState(() {});
    } on FirebaseException catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Lỗi khi tải ảnh: [${e.code}] ${e.message ?? ''}')),);
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> editDisplayName() async {
    ...
    if (user == null) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Chưa đăng nhập — không thể đổi tên.')),
      );
      return;
    }
              ...
              child: Container(
                ...
                child: Column(
                  children: [
                    ...
                    ListTile(contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 2),
                      leading: Container(
                        width: 48,
                        height: 48,
                        decoration: const BoxDecoration(
                ...
                onPressed: uploading
                  ...
                  try {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                          content:
                          Text('Đăng xuất thất bại: $e')),
                    );
                  }
                },
                style: ElevatedButton.styleFrom(
                  backgroundColor: Color(0xFF0A7C66),
